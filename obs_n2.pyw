...
DADOS = {
  ...
  "TV": {
            "COAXIAL": {
                "ÁREA NORMAL": "Cliente possui TV COAXIAL, sem reclamações o suficiente para acionar a Equipe de Rede na Região. ",
...
def getInternet():
    ...
    if "INTERNET" in cbProblema.get():
        for i, widget in enumerate([frames[1], frames[2]]): # Organizando os FRAMES de INTERNET
        ...
        for widget in (frames[1], frames[2]):
            widget.grid_remove()
    return situacaoNET
...
def getTV():
    ...
    if "TV" in cbProblema.get():
        frames[3].grid(column=0, row=6, padx=5, pady=5, columnspan=5, sticky="nswe")
...
WINDOW.title("SAC N2 - Software para Análise Nível 2 de Ordens de Serviço.")
...
# FRAME CABECALHO 
frames[0].grid(column=0, row=0, padx=10, pady=5, columnspan=5, sticky="nswe")
spacer1 = customtkinter.CTkLabel(frames[0], text="",)
spacer1.grid(column=4,row=0, padx=200)
spacer2 = customtkinter.CTkLabel(frames[2], text="", width=210)
...
# FRAME FINAL -----------------------------------------------------------------
frames[4].grid(column=0, row=8, padx=10, pady=5, columnspan=5, sticky="nswe")
var_end = IntVar()
...
btClear = customtkinter.CTkButton(master=frames[4], text="Limpar", command=clear, fg_color="red", hover_color="#d94545", width=120)
btCopy = customtkinter.CTkButton(master=frames[4], text="Copiar", command=ctrlc, width=120)
